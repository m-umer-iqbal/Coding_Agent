...
const { GENERATED_PROJECT_QUALITY_CONTRACT } = require('./agent/quality');
...
2. DATA PERSISTENCE — satisfy whatever ASR the spec names for durability
   ...
   • Provide a .env.example listing every required variable with a
     placeholder value. If you use dotenv or create a .env file, also
     create a .gitignore file that excludes .env and other secret files.
     Never leave credentials or environment secrets unprotected in the
     generated project's source tree.
...
sees has content in it.
  ...
  This applies to the REAL datastore too, not just an in-memory fallback.
  Creating the tables is not enough: a freshly created database renders
  every list screen empty and makes the app look broken on first run. So
  on startup, after the schema is in place, COUNT the rows of each table
  the UI reads and insert starter content when it is empty. Guard it on
  that count so restarts never duplicate rows, and keep the seed data in
  its own file rather than inline in the server, so it can be edited
...
════════════════════════════════════════════════════════════════════════
SHIP IT AS A DESKTOP APP — THE ELECTRON SHELL (web projects only)
════════════════════════════════════════════════════════════════════════
A web interface is delivered as a DESKTOP APPLICATION as well as a site.
The launcher opens an Electron window, not a browser tab, so a web build
that has no Electron main process is incomplete and task_complete is
rejected for it.

This is an ADDITION, never a replacement. Do not move the frontend, do
not change how it calls the API, do not turn off express.static, and do
not break \`npm start\` — the same server on the same port must still
serve the same site to a browser.

Write these three things:

1. electron/main.js — the main process. It must:
   - spawn the backend as a CHILD PROCESS (server.js / app.py / the same
     entry \`npm start\` uses) on 127.0.0.1, passing PORT through;
   - WAIT for it: poll GET /health (or /) every ~400ms until it answers,
     with a ~30s ceiling. Never open the window before the server is up —
     that is the classic blank-window bug;
   - open a BrowserWindow and loadURL('http://127.0.0.1:' + port). Load
     the HTTP URL, NOT loadFile() of the html: the frontend calls the API
     with relative paths, and a file:// origin breaks every one of them;
   - KILL THE CHILD on 'window-all-closed', on 'before-quit' and on
     SIGINT/SIGTERM. On Windows use \`taskkill /pid <pid> /T /F\`, which
     is the only thing that takes the whole tree down; elsewhere signal
     the process group. An orphaned server holds the port and the next
     launch fails;
   - show a REAL ERROR, not a blank window, when startup fails: capture
     the child's stdout/stderr and put the tail of it in
     dialog.showErrorBox with the likely causes (database down, port in
     use, dependencies missing, no .env);
   - use \`webPreferences: { nodeIntegration: false, contextIsolation:
     true, sandbox: true }\`. The page is ordinary web content and needs
     no Node access.
   - Run the child through process.execPath with ELECTRON_RUN_AS_NODE=1
     rather than a bare 'node', so it does not depend on node being on
     PATH.

2. package.json — an "electron" script and the dependency:
     "scripts": { "start": "node server.js", "electron": "electron electron/main.js" }
     "devDependencies": { "electron": "^33.2.0" }
   Keep "start" exactly as it was. The launcher runs \`npm run electron\`.

3. README.md — document both ways to run it: \`npm run electron\` for the
   desktop app and \`npm start\` + the URL for the browser.

Window defaults: a real title from the spec, 1280x800 with a 900x600
minimum, autoHideMenuBar, show:false until 'ready-to-show'. Open DevTools
only behind an explicit opt-in (e.g. ELECTRON_DEVTOOLS=1) — never by
default, since projects ship NODE_ENV=development in .env.example.

This applies ONLY to a web user interface. A CLI, a library or a mobile
app gets no Electron shell.
...
- The "summary" field of task_complete is USER-FACING OUTPUT. It must be
  a complete "How to run this project" guide structured exactly like this:
    ...
    ## Running the application (desktop app)
    \`\`\`
    npm install
    npm run electron
    \`\`\`
    <the Electron window opens and starts the backend itself>
...
${GENERATED_PROJECT_QUALITY_CONTRACT}`;
