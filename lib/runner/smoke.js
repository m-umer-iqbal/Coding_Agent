...
const { detectDesktopShell } = require('./launch');
...
function installDependencies(stack) {
  ...
  try {
    const output = execSync(stack.installCmd, {
      ...
      encoding: 'utf8',
      // Use the host OS default shell; do not assume Bash is installed.
      shell: true,
...
function writeRunGuide(result, plan) {
  ...
  if (desktop) {
    lines.push(
      'This project is a desktop application. It starts its own backend, so',
    ...
    lines.push(desktop.startCmd, '```', '');
  } else {
    lines.push('```bash', 'cd Project');
    if (stack.installCmd) lines.push(stack.installCmd);
...
` +
          `Then open **http://localhost:${stack.port}/**. The same server serves ` +
          'both the desktop window and the browser.'
...
function printRunBanner(result, plan) {
  ...
  console.log(`  ${desktop ? desktop.startCmd : stack.startCmd}`);

  if (desktop) {
    console.log('\n  A desktop window opens and starts the backend itself.');
    if (stack.port) {
      console.log(
        `  For the browser version instead: ${stack.startCmd}, ` +
        `then http://localhost:${stack.port}/`
      );
    }
  } else if (stack.port && plan && plan.ui && (plan.ui.kind === 'web' || plan.ui.kind === 'desktop')) {
