 ...
 */
/**
 * Is there a real Electron shell on disk?
 *
 * A web project is delivered as a desktop app, so the checklist is not
 * "did it write a file called main.js" but "can this actually open a
 * window": the main process has to create a BrowserWindow and load
 * something into it, and the manifest has to declare both the script
 * that runs it and the dependency it needs.
 *
 * Only web surfaces are held to this. A CLI, a library or a mobile app
 * has nothing to put in a window.
 */
function checkDesktopShell(plan, files, texts) {
  if (!DESKTOP_SHELL_REQUIRED) return [];
  if (!plan.ui || !plan.ui.required || plan.ui.kind !== 'web') return [];

  const missing = [];
  const entryName = DESKTOP_SHELL_ENTRY.toLowerCase();
  const entryPath = files.find(
    (f) => f.toLowerCase() === entryName || f.toLowerCase().endsWith('/' + entryName)
  );

  if (!entryPath) {
    missing.push(
      `no Electron main process — write ${DESKTOP_SHELL_ENTRY}, which starts ` +
      'the backend, waits for it and opens a BrowserWindow on it'
    );
  } else {
    const source = texts.get(entryPath) || ''; // lowercased by readProjectTexts
    if (!DESKTOP_SHELL_SIGNATURE.test(source)) {
      missing.push(
        `${entryPath} never creates a BrowserWindow, so no window can open`
      );
    } else if (!DESKTOP_SHELL_LOAD_SIGNATURE.test(source)) {
      missing.push(`${entryPath} creates a window but never loads the app into it`);
    }
  }

  const manifestPath = files.find(
    (f) => f.toLowerCase() === 'package.json' || f.toLowerCase().endsWith('/package.json')
  );
  if (manifestPath) {
    let pkg = {};
    try {
      pkg = JSON.parse(fs.readFileSync(path.join(PROJECT_DIR, manifestPath), 'utf8'));
    } catch (e) {
      pkg = {};
    }
    const scripts = pkg.scripts || {};
    if (!scripts[DESKTOP_SHELL_SCRIPT]) {
      missing.push(
        `package.json has no "${DESKTOP_SHELL_SCRIPT}" script — the launcher ` +
        `runs \`npm run ${DESKTOP_SHELL_SCRIPT}\` to open the desktop app`
      );
    }
    const declared =
    ...
    if (!declared) {
      missing.push('package.json does not list "electron" as a devDependency');
    }
  }
...
function checkPlan(plan) {
  ...
  const desktopMissing = checkDesktopShell(plan, files, texts);
...
function missingFromStatus(status) {
  const missing = [
    ...
    ...(status.desktopMissing || []).map((m) => `desktop app: ${m}`),
  ];
